...
int is_square_attacked(int sq, int attacking_side) {
    ...
    // === DEBUG CAVALIER ===
    if (knight_attacks[sq] & bitboards[pN]) {
        printf("\n=== DEBUG CRASH CAVALIER ===\n");
        printf("Case testee (Roi) : %d (E8=60 attendu)\n", sq);
        printf("Attaquant         : %s\n", (attacking_side == WHITE) ? "Blancs" : "Noirs");
        printf("Bitboard Cavaliers Ennemis : 0x%llx\n", (unsigned long long)bitboards[pN]);
        printf("Table d'attaque depuis E8  : 0x%llx\n", (unsigned long long)knight_attacks[sq]);
        printf("Collision (AND)            : 0x%llx\n", (unsigned long long)(knight_attacks[sq] & bitboards[pN]));
        
        if ((unsigned long long)bitboards[pN] != 0x42) printf(">>> ERREUR: Les cavaliers blancs ne sont pas en place (B1/G1) !\n");
        else printf(">>> INFO: Les cavaliers blancs sont bien en place (0x42).\n");
...
int main() {
    ...
    printf("Moteur C (Serveur) en attente de connexion sur le port %d...\n", PORT);

    if ((client_fd = accept(server_fd, (struct sockaddr *)&address, &addrlen)) < 0) { perror("Accept failed"); exit(EXIT_FAILURE); }
    printf("Client Python connecte !\n");
        ...
        if (parse_input_squares(buffer, &from, &to, &prom_char)) {
            ...
            if (move_found) {
                if (make_move(chosen_move, ALL_MOVES)) {
                    // 1. Le coup est valide, on prépare le board
                    char board_str[65];
                    serialize_board(board_str);
                    
                    // 2. On vérifie si ce coup a mis fin à la partie
                    ...
                    if (status == 0) {
                        ...
                        // MAT ! On envoie le board ET le signal de fin
                        // Format : "board:RN...kq game_over:checkmate"
                        sprintf(response, "board:%s game_over:checkmate", board_str);
                    } else {
                        // PAT
                        sprintf(response, "board:%s game_over:stalemate", board_str);
                    }
